...
const denegar = (motivo: string): Decision => ({
  permitido: false,
  motivo,
});
...
function puedeVer(actor: Actor, s: Solicitud): Decision {
  if (actor.rol === 'AUDITOR') return { permitido: true };
  if (s.sucursalId !== actor.sucursalId) {
    return denegar('La solicitud es de otra sucursal');
  }
...
function puedeCrear(actor: Actor): Decision {
  if (actor.rol === 'ANALISTA') {
    return denegar('Los analistas no pueden crear solicitudes');
  }
  return { permitido: true };
}
